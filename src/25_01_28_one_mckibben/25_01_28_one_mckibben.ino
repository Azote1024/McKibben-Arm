...
// 体積近似タイプ
// 0 目標位置と現在位置から体積を推定して線形補間
// 1 現在圧力のまま不変とする
// 2 無負荷のときの長さを用いる（これがデフォ）
#define TYPE_DELTAV 2
...
int calcTimeWidth(float p_current, float p_target, float v_current, float delta_v, float pipe_volume) {
  float p = p_current;
...
void loop() {
  ...
  time_width = calcTimeWidth(p_current, p_target, v_current, 0.0, PIPE_U);
