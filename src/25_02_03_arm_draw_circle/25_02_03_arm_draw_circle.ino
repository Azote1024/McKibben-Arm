...
// 関節の目標剛性値[N/mm]
#define JOINT_K 5000.0f

// 目標円軌道の半径
#define CIRCLE_PHI 150.0f

// 目標円軌道の周期[ms]
#define CIRCLE_T 4000.0f
...
void sampling() {
  ...
  Serial.print(",");
  Serial.print((int)getDegree(A4, 465)); //手先側
  Serial.println("");
}
...
int point_i = 0;
float target_x[20] = {
 435.47212047, 439.16673547, 416.9850903,  374.05161813, 319.35138314,
 262.92928817, 213.06521697, 174.46347438, 148.0378851,  132.19148114,
 124.87067375, 125.37800649, 135.0764254,  156.64361745, 192.20939446,
 241.24109658, 299.20059827, 357.70915963, 406.34012906, 435.47212047
};

float target_y[20] = {
  80.17908018,  118.78102608,  143.38928838,  153.13711742,  148.92858377,
  132.51387064,  105.75385802,   70.38384807,   28.31663545,  -17.75503325,
  -63.86337978, -104.76338607, -134.60595508, -148.16930449, -142.26315331,
 -116.79999805,  -75.1114069,   -23.34566803,   30.88616256,   80.17908018
};
void loop() {
  ...
  point_i++;
  if (point_i >= 20) point_i = 0;

  x_target = target_x[point_i];
  y_target = target_y[point_i];
